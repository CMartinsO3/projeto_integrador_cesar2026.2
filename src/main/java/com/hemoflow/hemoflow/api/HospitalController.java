...
public class HospitalController {
    ...
    public HospitalDTOs.Resposta criar(@Valid @RequestBody HospitalDTOs.Cadastro dto) {
        String base = dto.nome().trim().toUpperCase().replaceAll("\\s+", "_");
        String prefix = base.substring(0, Math.min(base.length(), 8));
        NoRedeDTOs.Cadastro noDto = new NoRedeDTOs.Cadastro(
                "H-" + prefix,
                dto.nome(),
                com.hemoflow.hemoflow.dominio.TipoNo.HOSPITAL
        );
        Hospital hospital = redeService.criarHospital(noDto);
        if (dto.latitude() != null && dto.longitude() != null) {
            hospital.setLatitude(dto.latitude());
            hospital.setLongitude(dto.longitude());
            hospital = redeService.salvarHospital(hospital);
        }
        return HospitalDTOs.Resposta.de(hospital);
    }
